...
def detect_signal(df):
    ...
    if curr_rsi <= 30:
        buy_signals.append(
            f"RSI 과매도 ({curr_rsi:.1f}) — 너무 많이 떨어져서 반등 가능성 높음"
        )
        buy_score += 1
    elif curr_rsi <= 40:
        buy_signals.append(
            f"RSI 저점 근접 ({curr_rsi:.1f}) — 약한 매수 구간 진입"
        )
        buy_score += 1

    if curr_close <= curr_lower:
        buy_signals.append(
            f"볼린저 하단 이탈 ({curr_lower:.2f}) — 과매도 구간, 단기 반등 기대"
        )
        buy_score += 1
    elif curr_close <= curr_lower * 1.02:
        buy_signals.append(
            f"볼린저 하단 근접 ({curr_lower:.2f}) — 지지선 테스트 중"
        )
        buy_score += 1

    if prev_macd < prev_macd_signal and macd > macd_signal:
        buy_signals.append(
            f"MACD 골든크로스 — 하락 모멘텀 끝, 상승 전환 신호"
        )
        buy_score += 2  # 강한 신호

    if prev_stoch_k < prev_stoch_d and stoch_k > stoch_d and stoch_k < 20:
        buy_signals.append(
            f"스토캐스틱 골든크로스 ({stoch_k:.1f}) — 과매도 후 반등 시작"
        )
        buy_score += 1

    if avg_volume > 0 and curr_volume >= avg_volume * 2.0:
        buy_signals.append(
            f"거래량 폭증 ({curr_volume/avg_volume:.1f}배) — 강한 매수세 유입"
        )
        buy_score += 2
    elif avg_volume > 0 and curr_volume >= avg_volume * 1.5:
        buy_signals.append(
            f"거래량 급증 ({curr_volume/avg_volume:.1f}배) — 매수세 증가"
        )
        buy_score += 1

    if prev_ma5 < prev_ma20 and curr_ma5 > curr_ma20:
        buy_signals.append(
            f"MA 골든크로스 (5일선이 20일선 돌파) — 중기 상승 전환"
        )
        buy_score += 2

    if curr_close > curr_ma20 and curr_close > curr_ma5:
        buy_signals.append(
            f"이동평균선 위 안착 — 상승 추세 유지 중"
        )
        buy_score += 1
    ...
    if curr_rsi >= 70:
        sell_signals.append(
            f"RSI 과매수 ({curr_rsi:.1f}) — 너무 많이 올라서 조정 가능성 높음"
        )
        sell_score += 1
    elif curr_rsi >= 60:
        sell_signals.append(
            f"RSI 고점 근접 ({curr_rsi:.1f}) — 약한 매도 구간 진입"
        )
        sell_score += 1

    if curr_close >= curr_upper:
        sell_signals.append(
            f"볼린저 상단 이탈 ({curr_upper:.2f}) — 과매수 구간, 단기 조정 가능"
        )
        sell_score += 1
    elif curr_close >= curr_upper * 0.98:
        sell_signals.append(
            f"볼린저 상단 근접 ({curr_upper:.2f}) — 저항선 테스트 중"
        )
        sell_score += 1

    if prev_macd > prev_macd_signal and macd < macd_signal:
        sell_signals.append(
            f"MACD 데드크로스 — 상승 모멘텀 약화, 하락 전환 신호"
        )
        sell_score += 2  # 강한 신호

    if prev_stoch_k > prev_stoch_d and stoch_k < stoch_d and stoch_k > 80:
        sell_signals.append(
            f"스토캐스틱 데드크로스 ({stoch_k:.1f}) — 과매수 후 하락 시작"
        )
        sell_score += 1

    if avg_volume > 0 and curr_volume < avg_volume * 0.5:
        sell_signals.append(
            f"거래량 급감 (평균의 {curr_volume/avg_volume:.1f}배) — 매수세 소멸"
        )
        sell_score += 1

    if prev_ma5 > prev_ma20 and curr_ma5 < curr_ma20:
        sell_signals.append(
            f"MA 데드크로스 (5일선이 20일선 하향 이탈) — 중기 하락 전환"
        )
        sell_score += 2

    if curr_close < curr_ma20 and curr_close < curr_ma5:
        sell_signals.append(
            f"이동평균선 아래 위치 — 하락 추세 진행 중"
        )
        sell_score += 1
